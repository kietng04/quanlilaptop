...
public class XemChiTietPhieuNhapDialog extends javax.swing.JDialog {
    ...
    public XemChiTietPhieuNhapDialog(java.awt.Frame parent, boolean modal, DTO_PhieuNhap phieunhapDTO) {
        super(parent, modal);
        this.phieunhap = phieunhapDTO;
        // JOptionPane.showMessageDialog(null, phieunhapDTO.getMaphieu());
        phieunhapBus = new BUS_PhieuNhap();
        chitietphieu = phieunhapBus.getChiTietPhieu_Type(phieunhapDTO.getMaphieu());
        // JOptionPane.showMessageDialog(null, "ÁA: "+chitietphieu);
    ...
    public void writePDFPhieuNhap(int maphieunhap) {
        ...
        try {
            ...
            company.add(new Chunk("Thoi gian in phieu: " + today));

            company.setAlignment(Element.ALIGN_LEFT);
            document.add(company);
            // Thêm tên công ty vào file PDF
            document.add(Chunk.NEWLINE);
            Paragraph header = new Paragraph("THONG TIN PHIEU NHAP");
            header.setAlignment(Element.ALIGN_CENTER);
            document.add(header);
            DTO_PhieuNhap pn = DAO_PhieuNhap.getInstance().selectById(maphieunhap + "");
            // Thêm dòng Paragraph vào file PDF

            Paragraph paragraph1 = new Paragraph("Ma phieu: " + pn.getMaphieu());
            String ncc = DAO_NhaCungCap.getInstance().selectById(pn.getManhacungcap() + "").getTenncc();
            Paragraph paragraph2 = new Paragraph("Nha cung cap: " + ncc);
            ...
            Paragraph paragraph3 = new Paragraph("Nhan vien tao: " + ngtao);
            // khoang cach 5 

            // paragraph3.add(new Chunk(createWhiteSpace(5)));
            paragraph3.add(new Chunk("-"));
            // paragraph3.add(new Chunk(createWhiteSpace(5)));
            paragraph3.add(new Chunk("Ma nhan vien: " + pn.getManguoitao()));
            Paragraph paragraph4 = new Paragraph("Thoi gian nhap: " + pn.getThoigiantao());
            ...
            table.addCell(new PdfPCell(new Phrase("Ten san pham")));
            table.addCell(new PdfPCell(new Phrase("Phien ban")));
            table.addCell(new PdfPCell(new Phrase("Gia")));
            table.addCell(new PdfPCell(new Phrase("So luong")));
            table.addCell(new PdfPCell(new Phrase("Tong tien")));
            for (int i = 0; i < 5; i++) {
            ...
            for (DTO_ChiTietPhieuNhap ctp : DAO_ChiTietPhieuNhap.getInstance().selectAll(maphieunhap + "")) {
                ...
                cell = new PdfPCell(
                        new Phrase(DAO_Product.getInstance().selectById(pb.getMasanpham() + "").getTensanpham()));
                table.addCell(cell);
                ...
                table.addCell(cell);
                cell = new PdfPCell(new Phrase(Formater.FormatVND(ctp.getDongia() * ctp.getSoluong()) + "VND"));
                table.addCell(cell);
            }
           JOptionPane.showMessageDialog(null, "NCC: "+ncc);
            

            document.add(table);
            document.add(Chunk.NEWLINE);

            Paragraph paraTongThanhToan = new Paragraph(new Phrase("Tong thanh tien: " + Formater.FormatVND(pn.getTongTien()) + "VND"));
            ...
            paragraph.add(new Chunk("Nguoi lap phieu"));
            paragraph.add(new Chunk(createWhiteSpace(30)));
            paragraph.add(new Chunk("Nhan vien nhan"));
            paragraph.add(new Chunk(createWhiteSpace(30)));
            paragraph.add(new Chunk("Nha cung cap"));

            Paragraph sign = new Paragraph();
            sign.setIndentationLeft(23);
            sign.add(new Chunk("(Ký  tên)"));
        ...
        jTable1.getSelectionModel().addListSelectionListener(new ListSelectionListener(){
            public void valueChanged(ListSelectionEvent event) {
                if (!event.getValueIsAdjusting() && jTable1.getSelectedRow() != -1) {
                    ...
                    int maphieunhap = Integer.parseInt(jTextField1.getText());
                    int maphienban = (int) jTable1.getValueAt(jTable1.getSelectedRow(), 0);
                    // JOptionPane.showMessageDialog(null, "Mã phiên bản: " + maphienban + " Mã phiếu nhập: " + maphieunhap);
                    chitietsanpham = ctspBus.getAllByMaPhieuNhapAndMaPBSP(maphieunhap, maphienban);
                      loadDataTableImei(chitietsanpham);
                }
            }
        });
    }
    ...
    public void loadDataTableImei(ArrayList<DTO_ChiTietSanPham> dssp) {
    ...
    public void loadDataTableChiTietPhieu(ArrayList<DTO_ChiTietPhieuNhap> ctPhieu) {
        ...
        // JOptionPane.showMessageDialog(null, "SIZE: " + ctPhieu);
    ...
    public void initPhieuNhap(DTO_PhieuNhap phieunhapDTO) {
        // JOptionPane.showMessageDialog(null,"DAY LA MA PHIEU NHAP"+ phieunhapDTO.getMaphieu());
